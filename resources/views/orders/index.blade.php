    ...
    <div class="card">
        ...
        <div class="table-container">
                                ...
                                <div class="action-menu-container">
                                    ...
                                    <div class="action-dropdown">
                                        ...
                                        @if($order->challan_file)
                                            <a href="{{ asset('uploads/' . $order->challan_file) }}" target="_blank">
                                        ...
                                        @endif
                                        @if(!in_array($order->status, ['Out for Delivery', 'Delivered', 'Cancelled', 'Returned', 'Invoiced']))
                                            <button type="button" style="color: #f87171;" onclick="cancelOrder('{{ $order->id }}', '{{ $order->order_number }}')">
                                                <i class="fas fa-times-circle" style="color: #f87171;"></i> Cancel Order
                                            </button>
                                        @endif
            ...
            <div id="paginationContainer" style="padding: 20px 0;">
                {{ $finalOrders->appends(request()->query())->links() }}
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function cancelOrder(id, orderNum) {
            if (confirm(`Are you sure you want to cancel order ${orderNum}?`)) {
                fetch(`${window.BASE_PATH}/orders/${id}/cancel`, {
                    method: 'PATCH',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
